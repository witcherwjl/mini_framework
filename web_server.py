...
class WSGIServer(object):
    ...
    def service_client(self, new_socket):
        ...
        if not file_name.endswith(".html"):
            ...
            #WSGI协议要求两个参数：字典，请求的所有信息；函数：用来发起响应,参数包括状态码，header等等
            environ = dict()
            environ["PATH_INFO"] = file_name
            body = self.application(environ, self.returnHttp_header)
    ...
    def returnHttp_header(self, status, headers):
        self.status = status
...
def main():
    ...
    #可以使用的路径加入./dynamic
    sys.path.append(conf_into["dynamic_path"])
    ...
    wsgi_server = WSGIServer(port, app, conf_into['static_path'])
